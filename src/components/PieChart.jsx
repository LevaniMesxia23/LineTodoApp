...
function PieChart() {
  ...
  const completedTasks = tasks.filter((task) => task.isComplated).length;
  const inProgressTasks = tasks.filter((task) => !task.isComplated).length;
  const stuckTasks = tasks.filter((task) => task.isImportant).length;
    ...
    datasets: [
      {
        ...
        backgroundColor: ["#80BC00", "#FFA400", "#6E7C7C"],
        borderWidth: 2,
        borderColor: "#FFFFFF",
      },
    ],
    ...
    <div className="flex items-center mt-10 lg:gap-20 md:flex-row lg:flex-row mb-12 flex-col gap-12">
      ...
      <div className="ml-5">
        <ul className="mt-2 flex flex-col gap-6">
          <li>
            <span style={{ color: "#80BC00" }}>●</span> {t("Completed")}:{" "}
            {completedTasks} ({((completedTasks / totalTasks) * 100).toFixed(1)}
            %)
          </li>
          <li>
